...
class WhisperMic:
    ...
    def __preprocess(self, data):
        ...
        if self.faster:
            return np.frombuffer(
                data, np.int16,
            ).flatten().astype(np.float32) / 32768.0, is_audio_loud_enough
        else:
    ...
    """
    Handles the task of getting the audio input via microphone.
    This method has been used for listen() method
    """
    ...
    """
    This method is similar to the __listen_handler() method
    but it has the added ability for recording the audio for a specified duration of time
    """
    ...
    """
    This method takes the recorded audio data,
    converts it into raw format and stores it in a queue.
    """
    ...
    def __transcribe(self, data=None, realtime: bool = False) -> None:
        ...
        if is_audio_loud_enough:
            ...
            if self.faster:  # faster_whisper returns an iterable object rather than a string
            ...
            """
            If the audio is not loud enough,
            we put None in the queue to indicate that we need to listen again or return None
            """
    ...
    """
    This method is similar to the listen() method,
    but it has the ability to listen for a specified duration,
    mentioned in the "duration" parameter.
    """
